...
window.addEventListener('load', () => {
    fetchEndpoint();
    addBotMessage("👋 Welcome! Send a message to start chatting with dat1 gpt-oss-120b model.");
});

// Fetch endpoint for display
async function fetchEndpoint() {
    try {
        const response = await fetch(`${API_BASE_URL}/api/endpoint`);
        const data = await response.json();
        endpointDisplay.textContent = `Send a message to start chatting with ${data.endpoint}`;
    } catch (error) {
